...
""" Return duall basis transformation matrix Ae. """
...
function assemble!(problem::Problem{Dirichlet}, time::Float64=0.0; auto_initialize=true)
    # FIXME: boilerplate
    if !isempty(problem.assembly)
        @warn(
            "assemble problem $(problem.name): problem.assembly is not empty and assembling, are you sure know what are you doing?"
        )
    end
...
function assemble!(assembly::Assembly, problem::Problem{Dirichlet}, element::Element, time::Float64)
