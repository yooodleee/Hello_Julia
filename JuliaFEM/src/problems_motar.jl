...
-----------
    dimension:
        dimension of surface, 1 for 2d problems (plane strain, plane stress, axisymmetric)
        and 2 for 3d problems. It not given, try to determine problem dimension from first
        element.
    rotate_normals
        if all surface elements are in cw order instead of ccw, this can be used to swap
        noraml directions so that normals point to outward of body.
    adjust
        for elasticity problems only; closes any gaps between usrfaces if found.
    dual_basis
        use bi-orthogonal basis when interpolating Lagrange multiplier space
    use_forwarddiff
        use forwarddiff to linearize contact constraints directly from weighted gap function.
    distval
        characteristic measure, contact pairs with distance over this value are skipped from 
        contact segmentation algorithm.
    linear_surface_elements
        convert quadratic surface elements to linear elements on the fly, notice that middle
        nodes are missing Lagrange multiplier.
    split_quadratic_slave_elements
        split quadratic surface elements to several linear sub-elements to get Lagrange multiplier
        to middle nodes also.
...
function Mortar()
    default_fields = []
    return Mortar(
        -1, false, false, false, false, Inf,
        true, true, true, 0.0, 1.0e-9, default_fields
    )
end
...
function assemble!(problem::Problem{Mortar}, time::Float64)
    ...
    if problem.properties.dimension == -1
        ...
        @info(
            "Assuming dimension of mesh tie surface is $dim. If this is wrong set is manaully using problem.properties.dimension"
        )
    end
...
function calculate_polygon_area(P)
    N_P = length(P)
    A = sum([norm(1/2 * cross(P[i] - P[1], P[mod(i, N_P) + 1])) for i = 2 : N_P])
    return A 
end
...
function diagnose_interface(problem::Problem{Mortar}, time::Float64)
    ...
    if props.split_quadratic_slave_elements
        ...
        if !props.linear_surface_elements
            @warn(
                "Mortar3D: split_quadratic_surfaces = true and linear_surface_elements = false maybe have unexpected behavior"
            )
        end
    ...
    for slave_element in slave_elements
        ...
        for master_element in master_elements
            ...
            P_area = calculate_polygon_area(P)
        ...
        S_prec = S_area_in_contact / S_area * 100.0
        push!(S_areas, S_area_in_contact)
        @info("Area of slave element in contact: $S_area_in_contact, it's $S_prec % of total element area")

        I_area += S_area_in_contact

    end # slave element done, contact virtual work ready
